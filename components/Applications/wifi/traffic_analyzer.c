    ...
    while (g_is_capturing_to_sd) {
        if (xQueueReceive(g_pcap_queue, &received_packet, pdMS_TO_TICKS(1000)) == pdPASS) {
            ...
            if (write_buffer) {
                ...
                storage_write_file(g_capture_filename, write_buffer, total_size, true);
                free(write_buffer);
    ...
    while (running) {
        if (!gpio_get_level(BTN_UP)) {
            ...
            if (g_is_capturing_to_sd) {
                ...
                strftime(g_capture_filename, sizeof(g_capture_filename), "/captura_%Y%m%d_%H%M%S.pcap", &timeinfo);
                ...
                storage_write_file(g_capture_filename, &global_header, sizeof(global_header), false);
